...
async function createGroupAPI(groupName, groupDescription, selectedUserList){
    try{
        ...
        return await axios.post('http://35.153.237.118:80/group/create-group/', grpObj, {headers: {'Authorization': localStorage.getItem('token')}});
    }
    catch(err){
        if(err){
...
async function getGroupInfoAPI(){
    try{
        return await axios.get(`http://35.153.237.118:80/group/get-info?groupId=${selectedGroup}`, {headers: {"Authorization": localStorage.getItem("token")}});
    }
    catch(err){
        if(err){
            alert(err.response.data.message);
...
async function fetchAllUsersAPI(){
    try{
        return axios.get('http://35.153.237.118:80/user/get-users', {headers: {"Authorization": localStorage.getItem("token")}});
    }
    catch{
        if(err){
            alert(err.response.data.message);
...
async function getUserGroupsAPI(){
    try{    
        return await axios.get('http://35.153.237.118:80/user/groups', {headers: {'Authorization': localStorage.getItem('token')}});
    }
    catch(err){
        if(err){
            alert(err.response.data.message);
...
async function updateGroupAPI(groupName, groupDescription){
    try{
        ...
        return await axios.put('http://35.153.237.118:80/group/update', reqObj, {headers: {'Authorization': localStorage.getItem('token')}});
    }
    catch(err){
        if(err){
            alert(err.response.data.message);
