...
async function getNewMessages(id, groupId){
    try{
        return await axios.get(`http://35.153.237.118:80:4000/chat/fetch?lastMessageId=${id}&groupId=${groupId}`, {headers: {"Authorization": localStorage.getItem("token")}});
    }
    catch(err){
        if(err){
...
async function sendNewMessage(groupID){
    try{
        ...
        if(message!==null && message!==undefined && message!==""){
            const response = await axios.post('http://35.153.237.118:80:4000/chat/send', {message: message, groupId: groupID}, {headers: {"Authorization": localStorage.getItem("token")}});
        }
    }
    catch(err){
        if(err){
